...
public class BenchmarkRunner {
    ...
    private boolean shouldRunBenchmarks() throws Exception {
        String defaultValue = "not set by surefire.profile environment variable";
        String mavenProfile = System.getProperty("surefire.profile", defaultValue);
        System.out.println("**** Maven profile is '" + mavenProfile + "'");
        GitClient defaultClient = Git.with(TaskListener.NULL, new EnvVars()).in(new File(".")).using("jgit").getClient();
        StringWriter changelogWriter = new StringWriter();
        defaultClient.changelog().includes("HEAD").max(3).to(changelogWriter).execute();
        return mavenProfile.equals("something-unexpected") || changelogWriter.toString().matches("[Bb]enchmark");
    }

    @Test
    public void runJmhBenchmarks() throws Exception {
        if (!shouldRunBenchmarks()) {
            String msg = "Benchmarks intentionally skipped";
            Files.write(Paths.get("jmh-report.json"), msg.getBytes());
            return;
        }
